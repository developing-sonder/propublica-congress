...
The Clients
============
The Client classes hold the logic required to connect to the API. Their main purpose is to mirror the ProPublica Congress API and handle the response. Each endpoint will return
an instance of a ``DevelopingSonder\PropublicaCongress\Http\Response::class``.

In addition to the Response class being returned,  the Client itself will retain information about the last request which can be accessed through ``$lastResponse`` variable.
This can be useful when you want to repeat a call or for pagination of data as some endpoint will only return one page of 20 results at the time.
...
.. code-block:: php
   ...
   $responseStatus = $client->lastResponse->status;

**Propublica's Pagination** returns page's of 20. The client classes contain an ``$page`` variable that allows you effectively designate a page of results to be returned. The
...
.. code-block:: php
   :linenos:
      ...
      $billManager = new Bills;

      // Return the Bills from the 115th Congress that passed in the Senate.
      $response = $billManager->recent('115', 'senate', 'passed');
      $bills = $response->results();

      // Return the next 20 recent votes
      $bills = $billManager->nextPage(true);

      // Create a Collection of Bills Resources
      $collection = BillsCollection::make($bills);
      $billSponsorName = $collection->first()->sponsor_name;
...
.. code-block:: php
   :linenos:
         ...
         // Return the Bills from the 115th Congress that passed in the Senate.
         $response = $billManager->recentByMember('R000595', 'passed');
         $bills = $response->results();

         // Alternatively you can pass an instance of a Member Resource
         $member = MemberClient::find('R000595')->results();
         $bills = $billManager->recentByMember($member, 'introduced')->results();